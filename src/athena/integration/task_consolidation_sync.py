...
class TaskConsolidationSync:
    ...
    def __init__(self, pattern_detection_window: int = 10, pattern_threshold: float = 0.6):
    ...
    def _detect_patterns(self) -> List[Dict[str, Any]]:
        ...
        for i in range(len(recent_tasks)):
            for j in range(i + 1, len(recent_tasks)):
                similarity = self._calculate_similarity(
                    recent_tasks[i]["content"], recent_tasks[j]["content"]
                )
                ...
                if similarity >= self.pattern_threshold:
                    pattern = {
                        ...
                        "matched_task_ids": [
                            recent_tasks[i]["task_id"],
                            recent_tasks[j]["task_id"],
                        ],
        ...
        if len(successful_tasks) >= 2:
            avg_duration = sum(t.get("duration_seconds", 0) for t in successful_tasks) / len(
                successful_tasks
            )
            patterns.append(
                {
    ...
    def _generate_recommendations(self, patterns: List[Dict[str, Any]], outcome: str) -> List[str]:
