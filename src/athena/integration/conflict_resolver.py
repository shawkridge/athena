...
class ConflictResolutionStore:
    ...
    def _ensure_schema(self) -> None:
        ...
        cursor.execute(
            ...
            CREATE TABLE IF NOT EXISTS conflict_resolutions (
                ...
                resolution_preference VARCHAR(50),  -- todo, plan, or strategy name
    ...
    async def get_preference_stats(
        ...
        """Get statistics about resolution preferences."""
        try:
            cursor = self.db.get_cursor()
            ...
            stats = {}
            for preference, count, avg_conf in rows:
                stats[preference] = {
                    "count": count,
...
# Global instances
_resolver: Optional[AdaptiveConflictResolver] = None
_store: Optional[ConflictResolutionStore] = None


def initialize(db: Database) -> None:
    """Initialize global instances."""
    global _resolver, _store
    _resolver = AdaptiveConflictResolver(db)
    _store = ConflictResolutionStore(db)
...
def get_resolution_store() -> ConflictResolutionStore:
    """Get global resolution store."""
    if _store is None:
        raise RuntimeError("Resolution store not initialized")
    return _store
