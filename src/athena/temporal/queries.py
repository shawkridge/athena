...
def _match_error_fix_pattern(
    ...
    for i in range(len(events) - 2):
        ...
        if (type1 == 'error' and
            ...
            confidence = _calculate_sequence_confidence(
                [e1, e2, e3],
                time_span,
                file_overlap=True
            )
        ...
        if error_count >= 2:
            ...
            for j in range(error_count, len(window)):
                if window[j].outcome == 'success':
                    ...
                    time_span = (debug_sequence[-1].timestamp - debug_sequence[0].timestamp).total_seconds()
    ...
    for session_id, session_files in session_events.items():
        ...
        if cwds:
            ...
            if len(common_prefix) > 0:
                time_span = (session_files[-1].timestamp - session_files[0].timestamp).total_seconds()
