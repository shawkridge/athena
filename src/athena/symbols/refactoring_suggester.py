...
from .technical_debt_analyzer import (
    DebtItem, DebtCategory, DebtSeverity, DebtScore, TechnicalDebtAnalyzer
)
...
class RefactoringSuggester:
    ...
    def _generate_steps(self, debt_item: DebtItem,
    ...
    def get_critical_path_suggestions(self) -> List[RefactoringSuggestion]:
        ...
        for suggestion in self.suggestions:
            if (suggestion.debt_item.severity in [DebtSeverity.CRITICAL, DebtSeverity.HIGH] and
                suggestion.total_effort > 8.0 and
                suggestion.total_impact > 0.5):
                critical.append(suggestion)
        return sorted(critical, key=lambda s: s.total_effort, reverse=True)

    def generate_roadmap(self, suggestions: Optional[List[RefactoringSuggestion]] = None) -> RefactoringRoadmap:
        ...
        total_impact = min(1.0, sum(s.total_impact for s in sorted_suggestions) / max(1, len(sorted_suggestions)))
    ...
    def get_refactoring_report(self) -> Dict:
        ...
        if not self.suggestions:
            return {
                ...
                "message": "No refactoring suggestions available"
            }
