...
class TestabilityAnalyzer:
    ...
    def analyze_symbol(self, symbol: Symbol) -> Optional[TestabilityScore]:
        ...
        suggestions = self._generate_suggestions(symbol, di_score, side_effect_score,
                                               coupling_score, ext_dep_score, issues)
    ...
    def _detect_issues(self, symbol: Symbol, code: str) -> List[TestabilityIssueDetail]:
        ...
        if 'global ' in code:
            issues.append(TestabilityIssueDetail(
                symbol=symbol,
    ...
    def _calculate_coverage_potential(self, symbol: Symbol, code: str,
                                     issues: List[TestabilityIssueDetail]) -> float:
    ...
    def _generate_suggestions(self, symbol: Symbol, di_score: float,
                            side_effect_score: float, coupling_score: float,
                            ext_dep_score: float,
                            issues: List[TestabilityIssueDetail]) -> List[str]:
        """Generate improvement suggestions."""
        suggestions = []
        
        if di_score < 60:
            suggestions.append("Use dependency injection to make dependencies explicit and mockable")
    ...
    def _calculate_metrics(self):
        ...
        highly_testable = len([s for s in self.scores if s.rating == TestabilityRating.HIGHLY_TESTABLE])
        testable = len([s for s in self.scores if s.rating == TestabilityRating.TESTABLE])
        moderately = len([s for s in self.scores if s.rating == TestabilityRating.MODERATELY_TESTABLE])
        ...
        critical_issues = sum(len([i for i in s.issues if i.severity == "critical"]) 
