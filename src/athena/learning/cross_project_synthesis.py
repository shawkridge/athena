...
class CrossProjectSynthesis:
    ...
    def get_universal_best_practices(
        self, agent_name: str, min_confidence: float = 0.8, time_window_days: int = 30
    ) -> List[Dict[str, Any]]:
        ...
        return sorted(
            practices, key=lambda x: (x["universally_applicable"], x["consistency"]), reverse=True
        )
    ...
    def transfer_knowledge_to_project(
        ...
        candidates = [o for o in source_outcomes if o.success_rate >= confidence_threshold]
    ...
    def detect_emerging_trends(self, time_window_days: int = 7) -> Dict[str, Any]:
        ...
        for agent in agents:
            current_rate = self.tracker.get_success_rate(
                agent, time_window_hours=time_window_days * 24
            )
            ...
            if delta > 0.1:
                trends["improving"].append(
                    {"agent": agent, "current_rate": current_rate, "improvement": delta}
                )
            elif delta < -0.1:
                trends["declining"].append(
                    {"agent": agent, "current_rate": current_rate, "decline": abs(delta)}
                )
            else:
                trends["stable"].append({"agent": agent, "success_rate": current_rate})
    ...
    def recommend_agent_focus(self) -> Dict[str, Any]:
        ...
        recommendations = {"highest_priority": None, "focus_agents": [], "reasoning": []}
        ...
        if trends["declining"]:
            ...
            recommendations["reasoning"].append(
                f"Declining: {declining[0]['agent']} (-{declining[0]['decline']:.1%})"
            )

        # Secondary focus: stable agents with room for improvement
        stable_with_potential = [a for a in trends["stable"] if a["success_rate"] < 0.8]
    ...
    def learning_velocity_analysis(self) -> Dict[str, Any]:
            ...
            velocity_data[agent] = {
                ...
                "velocity_category": (
                    ...
                    if velocity_6h > 0.1
                        ...
                        "improving_slow"
                        if velocity_6h > 0.01
                        else "stable" if velocity_6h > -0.01 else "declining"
                    )
                ),
            }
        ...
        return {
            ...
            "system_velocity": sum(v["velocity_6h"] for v in velocity_data.values())
            / len(velocity_data),
