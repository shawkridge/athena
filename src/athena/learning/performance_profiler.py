...
class PerformanceProfiler:
    ...
    def get_slowest_operations(
        ...
        filtered = [
            m for m in self._metrics_buffer
            if not agent_name or m.agent_name == agent_name
        ]
        ...
        for op_name, times in by_operation.items():
            operations.append({
                ...
                'optimization_priority': (sum(times) / len(times)) * len(times)  # Total time impact
            })
    ...
    def recommend_optimizations(self, agent_name: str) -> List[Dict[str, Any]]:
        ...
        for operation in slowest:
            if operation['optimization_priority'] > 5000:  # High impact
                recommendations.append({
                    'type': 'optimization',
                    'priority': 'high' if operation['optimization_priority'] > 10000 else 'medium',
                    'operation': operation['operation'],
                    'description': f"Optimize {operation['operation']} (avg: {operation['avg_time_ms']:.0f}ms)",
                    'potential_savings': operation['avg_time_ms'] * 0.3  # Estimate 30% improvement
                })
    ...
    def get_system_performance(self) -> Dict[str, Any]:
        ...
        agent_stats = {
            agent: self.get_performance_stats(agent)
            for agent in agents
        }

        avg_score = sum(
            s.get('performance_score', 0.5) for s in agent_stats.values()
        ) / len(agent_stats)
        ...
        return {
            ...
            'system_status': (
                'excellent' if avg_score > 0.95
                else 'good' if avg_score > 0.8
