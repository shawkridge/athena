...
class SpatialStore:
    ...
    def _ensure_schema(self):
        ...
        if not hasattr(self.db, "conn"):
            logger.debug(
                "SpatialStore: PostgreSQL async database detected. Schema management is handled by _init_schema()."
            )
            return
        ...
        cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_name='spatial_nodes' AND table_schema='public'"
        )
        ...
        # Indexes
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_spatial_nodes_project ON spatial_nodes(project_id, full_path)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_spatial_nodes_depth ON spatial_nodes(project_id, depth)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_spatial_nodes_type ON spatial_nodes(project_id, node_type)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_spatial_relations_from ON spatial_relations(project_id, from_path)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_spatial_relations_to ON spatial_relations(project_id, to_path)"
        )

        # Symbol node indexes
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_symbol_nodes_file ON symbol_nodes(project_id, file_path, line_number)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_symbol_nodes_kind ON symbol_nodes(project_id, symbol_kind)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_symbol_nodes_language ON symbol_nodes(project_id, language)"
        )
        cursor.execute(
    ...
    def get_neighbors(self, project_id: int, center_path: str, max_depth: int = 2) -> List[str]:
        """
        ...
        while queue:
            ...
            if depth < max_depth:
                ...
                """,
                    (project_id, current_path, project_id, current_path),
                    fetch_all=True,
                )
    ...
    def find_complexity_hotspots(
        self, project_id: int, threshold: float = 10.0, limit: int = 20
    ) -> List:
