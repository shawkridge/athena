    ...
    def get_schema_info(db_path: str) -> Dict:
        ...
        try:
            ...
            info['temporal_decay']['indices'] = [
                idx for idx in CompressionSchema.REQUIRED_INDICES['temporal_decay']
                if idx in all_indices
            ]

            info['consolidation']['indices'] = [
                idx for idx in CompressionSchema.REQUIRED_INDICES['consolidation']
                if idx in all_indices
            ]
            ...
            # Validate
            info['temporal_decay']['valid'] = (
                len(info['temporal_decay']['columns']) == len(CompressionSchema.REQUIRED_COLUMNS['temporal_decay'])
                and len(info['temporal_decay']['indices']) == len(CompressionSchema.REQUIRED_INDICES['temporal_decay'])
            )

            info['consolidation']['valid'] = (
                len(info['consolidation']['columns']) == len(CompressionSchema.REQUIRED_COLUMNS['consolidation'])
                and len(info['consolidation']['indices']) == len(CompressionSchema.REQUIRED_INDICES['consolidation'])
