...
from .models import Component, ContentType
...
class VisuospatialSketchpad:
    ...
    def add_directory(
        self, project_id: int, directory_path: str, context: Optional[str] = None
    ) -> int:
        ...
        metadata = {"directory_path": directory_path, "context": context, "type": "directory"}
    ...
    def update_file_context(self, project_id: int, file_path: str, context: str):
    ...
    def get_files_in_directory(self, project_id: int, directory_path: str) -> List[Dict]:
        ...
        with self.db.conn:
            ...
            """,
                (project_id, self.component.value, directory_path),
            ).fetchall()
    ...
    def get_recently_accessed_files(self, project_id: int, limit: int = 5) -> List[str]:
        """
        ...
        with self.db.conn:
            rows = self.db.conn.execute(
                ...
                LIMIT ?
            """,
                (project_id, self.component.value, limit),
            ).fetchall()
    ...
    def find_by_directory(self, project_id: int, directory: str) -> List:
        ...
        with self.db.conn:
            ...
            """,
                (project_id, self.component.value, directory, f"{directory}%"),
            ).fetchall()
            return [self._row_to_item(row) for row in rows]
    ...
    def get_spatial_neighbors(self, project_id: int, file_path: str) -> List:
        ...
        with self.db.conn:
            rows = self.db.conn.execute(
                ...
                AND json_extract(metadata, '$.file_path') != ?
            """,
                (project_id, self.component.value, f"{parent_dir}%", file_path),
            ).fetchall()
            return [self._row_to_item(row) for row in rows]
