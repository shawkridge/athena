...
def extract_patterns(
    ...
    from .pattern_validation import validate_pattern, rank_patterns_by_reliability, calculate_cluster_confidence
    ...
    for pattern in patterns:
        ...
        adjusted_confidence = (
            validation.confidence_adjusted *
            cluster_confidence_multiplier
        )
...
def _is_tdd_workflow(events: List[EpisodicEvent]) -> bool:
    ...
    for i in range(len(events) - 2):
        ...
        if ('test' in e1.content.lower() and
            _get_event_type_str(e2) in ['file_change', 'action'] and
            e3.outcome == 'success'):
            return True
...
def _is_error_recovery(events: List[EpisodicEvent]) -> bool:
    ...
    for i in range(len(events) - 1):
        ...
        if (_get_event_type_str(current) == 'error' and
            next_event.outcome == 'success'):
            return True
...
def extract_common_tags(events: List[EpisodicEvent]) -> List[str]:
    ...
    tech_keywords = ['auth', 'test', 'database', 'api', 'frontend', 'backend',
                     'jwt', 'sql', 'react', 'python', 'typescript', 'docker']
