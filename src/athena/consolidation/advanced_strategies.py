...
class AdvancedConsolidationStrategy:
    ...
    def extract_causal_patterns(
        ...
        for i in range(len(all_outcomes) - 1):
            ...
            sequences[sequence_key].append({
                'result': next_outcome.success_rate,
                'outcome_type': next_outcome.outcome
            })
    ...
    def extract_hierarchical_patterns(
        self,
        agent_name: str
    ) -> Dict[str, Any]:
        ...
        macro_stats = {
            'overall_success': sum(o.success_rate for o in history) / len(history) if history else 0,
            'consistency': 1.0 - ((max(o.success_rate for o in history) - min(o.success_rate for o in history)) if history else 0),
            'level': 'macro'
        }
